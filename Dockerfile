...
# run cd /tmp/tensorflow_pkg/ \
#     && ls 

RUN cp /tmp/tensorflow_pkg/tensorflow-${TENSORFLOW_VERSION}-cp27-cp27mu-linux_x86_64.whl /root

# Make sure it's built properly
RUN pip install --no-cache-dir /root/tensorflow-${TENSORFLOW_VERSION}-cp27-cp27mu-linux_x86_64.whl \
